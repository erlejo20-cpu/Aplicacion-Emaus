...
# En Android, necesitamos una ruta donde tengamos permisos de escritura
if os.environ.get('ANDROID_ARGUMENT'):
    data_dir = os.environ.get('PYTHON_SERVICE_ARGUMENT', base_dir)
else:
    data_dir = base_dir
...
try:
    ...
    # Si falla en Android por permisos, intentamos una ruta alternativa
    UPLOAD_FOLDER = os.path.join(data_dir, 'uploads')
    os.makedirs(UPLOAD_FOLDER, exist_ok=True)

# Asegurar que los archivos JSON iniciales existan para evitar errores en el primer inicio
if not os.path.exists(CONFIG_FILE):
    with open(CONFIG_FILE, "w") as f:
        json.dump({"username": "Usuario", "theme": "light", "font_size": 16, "profile_pic": ""}, f)
